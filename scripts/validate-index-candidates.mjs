...
async function check(path) {
  ...
  try {
    const response = await fetch(url, {
      redirect: "manual",
      headers: { "user-agent": "GameCastle-IndexGate/1.0", accept: "text/html,application/xhtml+xml" },
    ...
    const pass = response.status === 200 && html.length >= 1200 && !noindex && canonicalOk && links >= 2;
    return {
      path, status: pass ? "approved" : "quarantined", http_status: response.status,
      canonical_url: canonical ? absolute(canonical) : null, robots_allowed: !noindex,
      indexable: !noindex && canonicalOk, internal_link_count: links,
      content_bytes: Buffer.byteLength(html), reason: pass ? null : `failed: http=${response.status}, content=${html.length}, canonical=${canonicalOk}, noindex=${noindex}, links=${links}`,
    };
  } catch (error) {
...
const { data: rows, error } = await supabase.from("entities").select("entity_type,slug,created_at").eq("status", "active").gte("created_at", start.toISOString()).lt("created_at", end.toISOString()).order("created_at", { ascending: false }).limit(DAILY_LIMIT);
if (error) throw error;

const candidates = (rows ?? []).map((row) => ({ ...row, path: pathFor(row) })).filter((row) => row.path);
const results = [];
for (const candidate of candidates) results.push({ ...(await check(candidate.path)), source_created_at: candidate.created_at, updated_at: new Date().toISOString() });
...
console.log(JSON.stringify({ candidates: results.length, approved, quarantined }, null, 2));
