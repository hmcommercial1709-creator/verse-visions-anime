...
export function ExploreTopicPage({ page, language }: { page: ExplorePage; language: "en" | "ar" }) {
  ...
  return (
    <main dir={ar ? "rtl" : "ltr"} className="mx-auto max-w-6xl px-4 py-12 lg:px-6">
      ...
      >
        <a href={ar ? "/ar/anime" : "/"}>{ar ? "الرئيسية" : "Home"}</a> /{" "}
        <a href={`${prefix}/explore`}>{ar ? "استكشف" : "Explore"}</a> / <span>{copy.title}</span>
      </nav>
      <header className="rounded-3xl border border-primary/25 bg-gradient-to-br from-primary/15 via-card to-background p-7 md:p-10">
        <p className="text-xs font-semibold uppercase tracking-[.2em] text-primary">
          {ar ? "دليل GameCastle" : "GameCastle guide"}
        </p>
      ...
      {page.category === "wallpapers" && (
        <section className="mt-10 grid gap-5 sm:grid-cols-2">
          {wallpaperAssets.map((asset) => (
            ...
            >
              <img
                src={`/rewards/thumbs/${asset}`}
                alt={`${copy.title} — ${asset.replaceAll("-", " ")}`}
      ...
      {page.official?.length ? (
        <section className="mt-12 rounded-2xl border border-border p-6">
          <h2 className="font-display text-2xl font-bold">
            {ar ? "مصادر ومتابعة رسمية" : "Official sources and further checks"}
          </h2>
      ...
      <section className="mt-12">
        ...
        <div className="mt-5 space-y-4">
          {copy.faq.map(([q, a]) => (
            <details key={q} className="rounded-2xl border border-border bg-card/40 p-5">
      ...
      <section className="mt-12 rounded-3xl border border-primary/25 bg-primary/10 p-7">
        <h2 className="font-display text-2xl font-bold">
          {ar ? "تابع الاستكشاف" : "Keep exploring"}
        </h2>
        <div className="mt-4 flex flex-wrap gap-3">
          <a
            href={`${prefix}/explore`}
          ...
          >
            {ar ? "كل الأدلة" : "All guides"}
            <ArrowRight className="h-4 w-4" />
          </a>
          <a
            href="/rewards/anime-wallpapers"
            className="rounded-xl border border-border px-5 py-3 font-semibold"
          >
            {ar ? "هدية الخلفيات" : "Wallpaper gift"}
          </a>
        </div>
      </section>
    </main>
