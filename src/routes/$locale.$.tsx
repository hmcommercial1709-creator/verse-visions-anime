...
const ARABIC_SECTION_NAMES: Record<string, string> = {
  anime: "دليل الأنمي", article: "مقال وتحليل أنمي", character: "ملف شخصية أنمي",
  category: "قسم الأنمي", genre: "تصنيف الأنمي", studio: "استوديو الأنمي",
  gaming: "دليل الألعاب", "gaming-hub": "مركز الألعاب", store: "متجر GameCastle",
  explore: "دليل الاستكشاف", streaming: "دليل المشاهدة والبث",
};
...
function localizedMetadata(code: string, path: string) {
  ...
  if (code === "ar") {
    ...
    const title = `${topic} — ${kind} بالعربية | GameCastle Anime`;
    const description = `اكتشف ${topic} في ${kind} من GameCastle Anime: معلومات موثوقة، ترتيب واضح، نصائح عملية وروابط مفيدة لعشاق الأنمي والألعاب بالعربية.`;
    return { title, description, ogLocale: "ar_AR" };
  }
  const title = `${topic} — GameCastle Anime ${locale.english}`;
  const description = `Explore ${topic} in the ${locale.english} edition of GameCastle Anime, with useful guides, clear navigation and carefully organized anime and gaming resources.`;
  return { title, description, ogLocale: locale.hrefLang };
}
