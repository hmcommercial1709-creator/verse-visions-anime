      ...
      scripts: [
        ...
        },
        {
          ...
          children: JSON.stringify(
            faqSchema([
              {
                q: `How many episodes does ${a.title} have?`,
                a: `${a.title} runs for ${a.episodes} episodes across ${a.seasons} season(s).`,
              },
              {
                q: `What is the best watch order for ${a.title}?`,
                a: `Our watch-order section breaks down both the release order and the chronological order for ${a.title}, including which filler you can safely skip.`,
              },
              {
                q: `Is ${a.title} worth watching?`,
                a: `${a.tagline} This page covers the premise, arcs, characters and watch order so you can decide before you start.`,
              },
            ]),
          ),
        },
...
function AnimeDetail() {
  ...
  const similar = anime.similar
    .map((s: string) => animes.find((a) => a.slug === s))
    .filter(Boolean) as typeof animes;
  const alsoEnjoyed = recommendAnime(anime.slug, 4).filter(
    (a) => !anime.similar.includes(a.slug),
  );
    ...
    <article>
          ...
          <div className="grid gap-8 lg:grid-cols-[280px_1fr] items-start">
            ...
            <div>
              <div className="text-xs uppercase tracking-[0.22em] text-primary font-semibold">
                {anime.status} · {anime.year}
              </div>
              <h1 className="mt-2 font-display text-4xl lg:text-6xl font-bold tracking-tight">
                {anime.title}
              </h1>
              {anime.japaneseTitle && (
              ...
              <p className="mt-4 text-xl text-gradient font-semibold">
                {anime.tagline}
              </p>
                ...
                {anime.genres.map((g: string) => {
                  ...
                  return (
                    <Link
                      ...
                      to="/genre/$slug"
                      params={{ slug: g }}
                      className="rounded-full border border-border bg-secondary/60 px-3 py-1 text-xs hover:border-primary hover:text-primary"
                    >
                      {genre?.name || g}
                    </Link>
                  );
              ...
              <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3 max-w-xl">
                <Stat
                  ...
                  value={`${anime.rating.toFixed(1)}/10`}
                />
                <Stat
                  icon={Tv}
                  label="Episodes"
                ...
                <Stat
                  icon={Calendar}
                  label="Seasons"
                  value={String(anime.seasons)}
                />
                <Stat
                  icon={Building2}
                  label="Studio"
                  value={studio?.name || anime.studio}
                />
              </div>
              ...
              <div className="mt-6 flex flex-wrap gap-2">
                <Link
                  ...
                  search={{ ep: undefined }}
                  className="inline-flex items-center gap-2 rounded-lg bg-primary px-4 py-2.5 text-sm font-semibold text-primary-foreground glow-primary"
                >
      ...
      <div className="mx-auto max-w-7xl px-4 lg:px-6 grid gap-10 lg:grid-cols-[1fr_320px] mt-10">
        <div className="min-w-0">
          ...
          <SectionBlock id="watch-order" title="Watch order">
            <ol className="space-y-2">
              {anime.watchOrder.map((w: string, i: number) => (
                ...
                >
                  <span className="grid h-7 w-7 place-items-center rounded-full bg-primary/20 text-primary font-bold text-sm">
                    {i + 1}
                  </span>
                  <span>{w}</span>
                </li>
            ...
            <SectionBlock
              ...
              title={
                <span className="flex items-center gap-2">
                  <Tv className="h-5 w-5 text-primary" /> Episode guides
                </span>
              }
            >
                ...
                {eps.map((e) => (
                  ...
                  >
                    <div className="text-xs uppercase tracking-[0.2em] text-primary">
                      Episode {e.number} · {e.arc}
                    </div>
                    <div className="mt-1 font-display text-lg font-bold group-hover:text-gradient">
                      {e.title}
                    </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {cast.map((c) => (
                  <Link
                    ...
                    to="/character/$slug"
                    params={{ slug: c.slug }}
                    className="rounded-xl border border-border/60 p-4 hover:border-primary/60 bg-card/50"
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className="h-12 w-12 rounded-full shrink-0"
                        style={{
                          background: `linear-gradient(135deg, ${c.accent}, #000)`,
                        }}
                      />
                      <div className="min-w-0">
          ...
          <SectionBlock id="quotes" title="Quotes to remember">
            <div className="space-y-3">
              {anime.quotes.map((q: any, i: number) => (
                ...
                >
                  <div className="italic text-lg text-foreground/90">
                    "{q.line}"
                  </div>
          ...
          <SectionBlock
            id="facts"
            title="Fun facts, hidden details & easter eggs"
          >
            <ul className="space-y-2">
              {anime.facts.map((f: any, i: number) => (
                ...
                >
                  <div className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-primary/20 text-primary text-xs font-bold">
                    {i + 1}
                  </div>
          ...
          <SectionBlock
            ...
            title={
              <span className="flex items-center gap-2">
                <Music className="h-5 w-5 text-accent" /> Soundtrack & openings
              </span>
            }
          >
            <div className="grid gap-2 sm:grid-cols-2">
              {anime.soundtrack.map((s: any, i: number) => (
                <div
                  key={i}
                  className="rounded-lg border border-border/60 bg-card/50 p-3 flex items-center gap-3"
                >
          ...
          <SectionBlock
            ...
            title={
              <span className="flex items-center gap-2">
                <Users className="h-5 w-5 text-accent" /> Voice cast
              </span>
            }
          >
              ...
              <table className="w-full text-sm">
                <thead className="text-xs uppercase text-muted-foreground bg-secondary/40">
                  <tr>
                    ...
                    <th className="text-left p-3">Japanese</th>
            ...
            >
              <ul className="grid gap-2 sm:grid-cols-2">
                {anime.awards.map((a: any, i: number) => (
                  <li
                    key={i}
                    className="rounded-lg border border-gold/30 bg-gold/5 p-3 text-sm"
                  >
                    {a}
                  </li>
                ))}
          ...
          >
            <div className="space-y-2">
              {anime.faq.map((f: any, i: number) => (
                <details
                  key={i}
                  className="group rounded-xl border border-border/60 bg-card/50 p-4"
                >
          ...
          <SectionBlock id="conclusion" title="Conclusion">
            <p className="text-lg leading-relaxed">
              {anime.title} earned its spot in the GameCastle Anime library
              because it did what most anime doesn't: it kept its promises. The
              arcs paid off. The characters grew. The world got bigger the
              longer we sat with it. If you're picking your next weekend of
              viewing, this is one worth clearing the calendar for.
            </p>
          </SectionBlock>
          ...
          {similar.length > 0 && (
            <SectionBlock id="similar" title="Similar anime you'll enjoy">
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {similar.map((s) => (
                  <AnimeCard key={s.slug} anime={s} size="sm" />
                ))}
              </div>
            </SectionBlock>
          )}
...
function SectionBlock({
  ...
  return (
    <section id={id} className="my-10 scroll-mt-24">
      <h2 className="font-display text-2xl sm:text-3xl font-bold mb-4 flex items-center gap-2">
        {title}
      </h2>
      {children}
    </section>
