...
export const Route = createFileRoute("/anime/")({
  // page is omitted on page 1 so /anime is itself canonical.
  //
  // This route is the parent of /anime/$slug and /anime/$slug/$section, so
  // normalising page to 1 here did not just redirect /anime → /anime?page=1:
  // it forced ?page=1 onto every descendant, giving 148 of the sitemap's URLs
  // a redirect — a third of the file — including detail pages where a page
  // parameter is meaningless. Google files "Page with redirect" as not
  // indexed, so those URLs could never be indexed at their advertised address.
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white px-4 py-12 max-w-7xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        <a
          href="/"
          className="text-xs bg-slate-900 border border-slate-800 px-4 py-2 rounded-lg hover:border-pink-500 transition"
        >
          ← Back to Home
        </a>
      </div>
      ...
      <div className="flex justify-between items-center mt-8 pt-4 border-t border-slate-900">
        {page > 1 ? (
          <a
            href={page - 1 === 1 ? "/anime" : `/anime?page=${page - 1}`}
            className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-lg hover:border-pink-500 text-sm"
          >
            ← Previous
          </a>
        ) : (
