  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        {
          property: "og:url",
          content: `https://gamecastle.store/anime/${anime.slug}/episode/${ep.number}`,
        },
      ...
      links: [
        {
          rel: "canonical",
          href: `https://gamecastle.store/anime/${anime.slug}/episode/${ep.number}`,
        },
      ],
      scripts: [
        {
...
function EpisodePage() {
  ...
  const recapAdPlan = planInArticleAds([ep.recap.length], {
    interval: 2,
    startAt: 2,
    max: 14,
  });
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-4xl px-4 lg:px-6 pt-14 pb-10">
          ...
          <p className="mt-2 text-[11px] text-muted-foreground">
            GameCastle Anime links to official streaming platforms only — 1080p
          ...
          <Block title="Major events">
            <ul className="space-y-2">
              {ep.majorEvents.map((e, i) => (
                ...
                >
                  <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-primary/20 text-primary text-xs font-bold">
                    {i + 1}
                  </span>
                  <span>{e}</span>
                </li>
          ...
          <div className="my-12 grid gap-3 sm:grid-cols-2">
            {prev ? (
              <Link
                ...
                params={{ slug: anime.slug, num: String(prev.number) }}
                className="rounded-xl border border-border/60 bg-card/50 p-4 hover:border-primary/60"
              >
            ...
            {next ? (
              <Link
                ...
                className="rounded-xl border border-border/60 bg-card/50 p-4 hover:border-primary/60 text-right"
              >
