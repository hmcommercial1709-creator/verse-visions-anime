...
export const Route = createFileRoute("/compare/anime/$pair")({
  loader: async ({ params }) => {
    const data = await loadComparison("anime", params.pair);
  ...
  head: ({ loaderData, params }) => {
    ...
    return {
      meta: [
        ...
        ...(requested &&
        comparisonPath("anime", requested[0], requested[1]) !== loaderData.canonical
  ...
  component: AnimeComparisonRoute,
});

function AnimeComparisonRoute() {
  const data = Route.useLoaderData();
  return <ComparisonPage type="anime" data={data} />;
}
