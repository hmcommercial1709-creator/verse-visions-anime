...
export const Route = createFileRoute("/sitemap-page")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Every page on GameCastle Anime, organized by section.",
      },
      { property: "og:title", content: "Sitemap · GameCastle Anime" },
      { property: "og:description", content: "All pages on GameCastle Anime." },
    ],
    links: [
      { rel: "canonical", href: "https://gamecastle.store/sitemap-page" },
    ],
  }),
  component: () => (
      ...
      <div className="mt-8 grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        <Section title="Anime">
          {publishedAnime().map((a) => (
            <Item key={a.slug} to="/anime/$slug" params={{ slug: a.slug }}>
              {a.title}
            </Item>
          ))}
        </Section>
        <Section title="Genres">
          {populatedGenres().map((g) => (
            <Item key={g.slug} to="/genre/$slug" params={{ slug: g.slug }}>
              {g.name}
            </Item>
          ))}
        </Section>
        <Section title="Studios">
          {populatedStudios().map((s) => (
            <Item key={s.slug} to="/studio/$slug" params={{ slug: s.slug }}>
              {s.name}
            </Item>
          ))}
        </Section>
        <Section title="Characters">
          {publishedCharacters().map((c) => (
            <Item key={c.slug} to="/character/$slug" params={{ slug: c.slug }}>
              {c.name}
            </Item>
          ))}
        </Section>
        <Section title="Articles">
          {publishedArticles().map((a) => (
            <Item key={a.slug} to="/article/$slug" params={{ slug: a.slug }}>
              {a.title}
            </Item>
          ))}
        </Section>
